...
while:
    ...
    li $a0,250   # 1000 ms = 1 s
    jal delay      # faz o delay

    
if: beq $s1,0,else  # if ( flag == 0) soma

    and $t2,$s0,0x8  # isola o ultimo bit n=4
    srl $t2,$t2,3  # mexe ele para a posicao 1
    
    sll $s0,$s0,1 # deslocamento de 1 bit da sequencia
    or $s0,$s0,$t2    # or com o bit que foi tirado
    j endif

else: 
    srl $s0,$s0,1
ifaneld:bne $s0,0,endif
    li $s0,8 
